...
const TableRow = ({ product, handleDelete, handleAddvertise, index }) => {
  ...
  return (
    <tr>
      ...
      <td className="px-5 font-bold border-b border-gray-200 bg-white text-sm">
        <p className="text-gray-900 whitespace-no-wrap">${sellprice}</p>
      </td>
      ...
      <td className="px-5 font-bold border-b border-gray-200 bg-white text-sm">
        <p className="text-gray-900 whitespace-no-wrap">{condition}</p>
      </td>
      <td className="px-5 font-bold border-b border-gray-200 bg-white text-sm capitalize">
        <p
          className={`${
            product?.status === "sold"
              ? "bg-red-200 text-[#C25934] "
              : "text-green-900 bg-green-200"
          } text-center font-semibold rounded-full px-2 py-1`}
        >
          {status}
        </p>
      </td>
      ...
      <td className="px-5 font-bold border-b border-gray-200 bg-white text-sm capitalize">
        {product?.status === "available" && (
          <>
            {!product?.advertise ? (
              <p
                onClick={() => handleAddvertise(product)}
                className="text-blue-900 bg-blue-200 text-center font-semibold rounded-full px-2 py-1 hover:cursor-pointer"
              >
                Addvertise
              </p>
            ) : (
              <p className="text-fuchsia-900 bg-fuchsia-200 text-center font-semibold rounded-full px-2 py-1">
                Running...
              </p>
            )}
          </>
